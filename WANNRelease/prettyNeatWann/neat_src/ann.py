...
def getNodeOrder(nodeG, connG):
  ...
  """
    ...
    # individual node ids for input and output nodes
    lookup = node[0, :].astype(int)

    # logging.info("Before:")
    # logging.info(src)

    # tg: not sure what is this doing
    # tg: it is reinitializing the node ids if there are
    # gaps in between
    ...
    # tg: N x N where N - sum of all bias, input,
    # hidden and output nodes
    wMat = np.zeros((np.shape(node)[1], np.shape(node)[1]))
    # tg: assign weight values
    wMat[src, dest] = conn[3, :]

    # tg: what does this mean - ends up being a 0 dim matrix.
    # tg: are these for the hidden layer connections?
    connMat = wMat[nIns + nOuts:, nIns + nOuts:]
    connMat[connMat != 0] = 1
    #logging.info("connMat:\n" + str(connMat))

    # Topological Sort of Hidden Nodes
    edge_in = np.sum(connMat, axis=0)
    # Start with nodes with no incoming connections
    Q = np.where(edge_in == 0)[0]
    #logging.info("Q:\n" + str(Q))

    for i in range(len(connMat)):
        # tg: First if loop can be outside the for loop
    ...
    # tg: Q contains the topologically sorted hidden nodes.
...
def act(weights, aVec, nInput, nOutput, inPattern):
  ...
  If the variable weights is a vector it is turned into a
  square weight matrix.
  
  Allows the network to return the result of several samples
  at once if given a matrix instead of a vector of inputs:
