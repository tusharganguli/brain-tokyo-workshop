...
def master():
  ...
  """
    logging.info(str(rank) + "master")

    global fileName, hyp
    # Create an instance of DataGatherer
...
def gatherData(data, wann, gen, hyp, savePop=False):
  ...
  """
    logging.info(str(rank) + "gatherData")
...
def checkBest(data):
  ...
  * This is a bit hacky, but is only for data gathering, and not optimization
  """
    logging.info(str(rank) + "checkBest")
...
def batchMpiEval(pop, sameSeedForEachIndividual=True):
  ...
  """
    logging.info(str(rank) + "batchMpiEval")
...
def stopAllWorkers():
  ...
  """
    logging.info(str(rank) + "stopAllWorkers")
    global nWorker
    nSlave = nWorker - 1
...
def mpi_fork(n):
  ...
  """
    global rank
    logging.info(str(rank) + "Inside mpi_fork")
...
def main(argv):
  ...
  """
    logging.info(str(rank) + "Inside main")
